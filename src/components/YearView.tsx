...
const YearView: React.FC<YearViewProps> = ({ onMonthClick, onDayClick }) => {
  ...
  const getYearStats = () => {
    const allEvents = scheduledServices.filter(service => {
      const serviceDate = new Date(service.scheduledDate)
  ...
  const getEventDensityColor = (eventCount: number) => {
    ...
    if (eventCount <= 2) return 'bg-blue-100'
    if (eventCount <= 5) return 'bg-blue-300'
    if (eventCount <= 10) return 'bg-blue-500'
    return 'bg-blue-700'
  }
  ...
  return (
    <div className="space-y-6">
      {/* Year Statistics */}
      <Card>
        <CardHeader className="bg-off-white border-b border-gold">
            ...
            >
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <Badge 
                    variant="outline" 
                    className={monthStats.totalEvents > 0 ? 'bg-gold text-dark-grey' : 'bg-light-grey text-medium-grey'}
                  >
              ...
              <CardContent className="pt-0">
                  ...
                  {Array.from({ length: 42 }, (_, i) => {
                    ...
                    return (
                      <div
                        ...
                        className={`
                          ...
                          ${!isCurrentMonth 
                            ? 'text-gray-300' 
                            : isCurrentDay 
                            ? 'bg-gold text-dark-grey font-bold' 
                            : dayEvents.length > 0
                      ...
                      >
                        {cellDate.getDate()}
                        {dayEvents.length > 0 && (
                          <div className="absolute bottom-0 right-0 w-1.5 h-1.5 bg-gold rounded-full"></div>
                        )}
                      </div>
                    )
                  })}
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium text-dark-grey">{monthStats.totalEvents}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-medium-grey">Hours:</span>
                    <span className="font-medium text-dark-grey">{monthStats.totalHours}h</span>
                  </div>
                  <div className="flex items-center justify-between">
