    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <p className="text-medium-grey font-space-grotesk uppercase tracking-wide">LOADING SECURITY DATA...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="w-6 h-6 bg-gold flex items-center justify-center">
                <Activity className="h-4 w-4 text-dark-grey" />
              </div>
            </div>
        ...
        <Card className="bg-white border-2 border-light-grey">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="w-6 h-6 bg-gold flex items-center justify-center">
                <Shield className="h-4 w-4 text-dark-grey" />
              </div>
            </div>
            <div className="text-3xl font-bold text-dark-grey mb-1 font-space-grotesk">
        ...
        <Card className="bg-white border-2 border-light-grey">
          <CardContent className="p-6">
            ...
            <div className="text-3xl font-bold text-red-600 mb-1 font-space-grotesk">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-2">
              <select
                ...
                className="px-3 py-1 border-2 border-light-grey bg-white text-dark-grey font-space-grotesk text-sm"
              >
          ...
          {filteredEvents.length === 0 ? (
            ...
            <div className="max-h-96 overflow-y-auto">
              {filteredEvents.map((event: SecurityEvent) => (
                <div 
                  key={event.id}
                  className="flex items-center justify-between p-4 border-b border-light-grey hover:bg-off-white transition-colors"
                >
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <div className="font-bold text-dark-grey font-space-grotesk text-sm uppercase">
          ...
          <CardContent className="p-6">
            {Object.keys(securityStats?.loginAttemptsByEmail || {}).length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-medium-grey font-space-grotesk">No login attempts recorded</p>
              </div>
            ) : (
              <div className="space-y-3">
                {Object.entries(securityStats?.loginAttemptsByEmail || {}).map(([email, count]: [string, any]) => (
                  <div key={email} className="flex items-center justify-between">
                    <span className="text-dark-grey font-space-grotesk">{email}</span>
                    <Badge className="bg-gold text-dark-grey font-bold">
                      {count} attempts
                    </Badge>
                  </div>
                ))}
          ...
          <CardContent className="p-6">
            {(securityStats?.rateLimitedIPs || []).length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-medium-grey font-space-grotesk">No blocked IP addresses</p>
              </div>
            ) : (
              <div className="space-y-3">
                {(securityStats?.rateLimitedIPs || []).map((ip: string) => (
                  <div key={ip} className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-dark-grey font-space-grotesk font-mono">{ip}</span>
                    </div>
